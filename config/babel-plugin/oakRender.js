    ...
    return {
        visitor: {
            Program(path, state) {
                ...
                const tsPage = (/oak-general-business\/(lib|src)/.test(rel) && /(pages|components)[\w|\W]+index\.(web.ts|ts)$/.test(rel)) ||
                    (!/node_modules/.test(rel) && /(pages|components)[\w|\W]+index\.(web.ts|ts)$/.test(rel));
                const jsPage = (/oak-general-business\/(lib|src)/.test(rel) && /(pages|components)[\w|\W]+index\.(web.js|js)$/.test(rel)) ||
